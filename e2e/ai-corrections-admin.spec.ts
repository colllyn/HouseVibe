...
test.describe("AI Corrections Admin Dashboard", () => {
  test("1. admin can view dashboard with stat cards and all data sections", async ({ page }) => {
      ...
      await supabase.from("ai_correction_logs").insert([
        {
          ...
          original_output: JSON.stringify({ price: "5000", description: "nice" }),
          corrected_output: JSON.stringify({ price: "5500", description: "beautiful" }),
          diff: JSON.stringify([
        ...
        },
        {
          ...
          original_output: JSON.stringify({ title: "apartment" }),
          corrected_output: JSON.stringify({ title: "luxury apartment" }),
          diff: JSON.stringify([
    ...
    try {
      ...
      // Navigate to AI corrections dashboard
      ...
      await expect(page.getByText("负反馈率")).toBeVisible({ timeout: 5000 });

      // Should see data sections
      await expect(page.getByText("高频被修改字段")).toBeVisible({ timeout: 5000 });
      await expect(page.getByText("常见值修正映射")).toBeVisible({ timeout: 5000 });
      await expect(page.getByText("各功能负反馈率")).toBeVisible({ timeout: 5000 });
      await expect(page.getByText("各 Prompt 版本纠错率")).toBeVisible({ timeout: 5000 });
      await expect(page.getByText("用户偏好学习效果")).toBeVisible({ timeout: 5000 });

      // Should see table with field data
  ...
  test("3. admin can filter by feature", async ({ page }) => {
    ...
    if (wsId) {
      await supabase.from("ai_correction_logs").insert([{
        ...
        request_id: crypto.randomUUID(), entity_type: "property",
        entity_id: crypto.randomUUID(), prompt_version: "1", model_name: "deepseek",
        original_output: JSON.stringify({ price: "3000" }),
        corrected_output: JSON.stringify({ price: "3200" }),
        diff: JSON.stringify([
  ...
  test("7. filter state persists across data refresh", async ({ page }) => {
    ...
    if (wsId) {
      await supabase.from("ai_correction_logs").insert([{
        ...
        request_id: crypto.randomUUID(), entity_type: "property",
        entity_id: crypto.randomUUID(), prompt_version: "1", model_name: "deepseek",
        original_output: JSON.stringify({ price: "4000" }),
        corrected_output: JSON.stringify({ price: "4200" }),
        diff: JSON.stringify([
