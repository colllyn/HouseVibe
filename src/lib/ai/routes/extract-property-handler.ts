...
export function createExtractPropertyHandler(
  ...
  return async function POST(request: NextRequest) {
    ...
    try {
      ...
      // 9. Call Provider with REDACTED text — minimal DTO, no identity
      ...
      let result: PropertyExtractionResult;
      try {
        result = await provider.extractProperty(providerInput, request.signal);
      } catch (providerErr) {
      ...
      if (reservationId && idemKey) {
        await settleQuota({
          ...
          model: "", // Provider will inject model name
        });
        reservationId = null;
      }
